...
def Ornstein_Uhlenbeck_2D(initial,N_data, T, dt, n_dim, B, Sigma, seeds,IC_):
    ...
    if IC_=='uniform':
        ...
        xIC[1,:] = np.random.uniform(-1.5, 1.5, N_data)
    elif IC_=='value':
...
savedir = 'C:\\Users\\yliu1\\OneDrive - Middle Tennessee State University\\cyberduck\\sde_learning\\final_code\\2D_SO\\'
make_folder(savedir)

N_sample_path = 3000000
...
learning_rate = 0.01
FN = FN_Net(x_dim*2, x_dim, 40).to(device)
FN.zero_grad()
...
ax.set_title(f'$x_0^{(1)}$ = {true_init[0]:.2f}' , fontsize=16)
...
ax.set_xlabel('Time', fontsize=16)
ax.set_ylabel('Value', fontsize=16)
ax.set_title(f'$x_0^{(2)}$ = {true_init[1]:.2f}' , fontsize=16)
...
fig, axes = plt.subplots(1, x_dim, figsize=(x_dim* 8, 6))
...
for i in range(x_dim):
    ...
    if i ==0:
        ax.axvline(x=true_samples[0,i], color='blue')
        ax.set_xlim([-0.55,-0.45])
        ax.set_xticks(np.linspace(-0.54,-0.46,5))
        
        # Plot histogram with filled color for the current dimension
        counts, bins, _ = ax.hist(prediction[:, i], bins=2, density=True, alpha=0.5, color=(0.625, 0.625, 0.625), histtype='stepfilled', edgecolor='black')

        # counts, bins, _ = ax.hist(true_samples[:, i], bins=5, density=True, alpha=0.5, color=(1.0, 0.5, 0.5), histtype='stepfilled', edgecolor='red')
        # # Compute kernel density estimation for the current dimension
        # bandwidth = 1e-12
        # kde = gaussian_kde(true_samples[:, i].T, bw_method=bandwidth)
        # x_vals = np.linspace(-0.54, -0.46, 400)
        # pdf_vals = kde(x_vals)
        ...
        x_vals = np.linspace(-0.56, -0.44, 100)
        pdf_vals = kde(x_vals)
         # Plot the kernel density estimate
        ax.plot(x_vals, pdf_vals, color='blue', linewidth=1.4)
    ...
    # ax.set_xlabel(f'X_{i+1}', fontsize=16)
    ax.set_ylabel('pdf', fontsize=16)
    ax.set_title(f'Marginal Density for $X^{(i+1)}$', fontsize=16)
    ax.legend(['Reference', 'Pred'])
    # Increase font size of tick labels
    ax.tick_params(axis='both', labelsize=14)
