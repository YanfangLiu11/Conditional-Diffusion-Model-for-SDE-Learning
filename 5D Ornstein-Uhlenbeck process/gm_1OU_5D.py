...
B = np.array([[0.2, 1.0, 0.2, 0.4, 0.2], 
              [-1.0, 0.0, 0.2, 0.8, -1.0],
               ...
               [-0.6, 0.0, 1.2, -0.2, 0.6],
               [0.2, 0.2, 0.6, 0.4, 0.0]])
diagonal_elements = [0, 0, 0, 1, 0]
Sigma = np.diag(diagonal_elements)
x_dim = 5
...
savedir = 'C:\\Users\\yliu1\\OneDrive - Middle Tennessee State University\\cyberduck\\sde_learning\\final_code\\1OU_5D\\'
make_folder(savedir)
...
diff_scale = np.array([10, 10, 10, 4, 10])
...
for i in range(x_dim):
    ...
    if i ==0:
        ...
        x_vals = np.linspace(0.28, 0.34, 100)
    elif  i ==1:
        bandwidth = 1e-6
        kde = gaussian_kde(true_samples[:, i].T, bw_method=bandwidth)
        x_vals = np.linspace(-0.22, -0.18, 100)
    elif  i ==2:
        bandwidth = 1e-6
        kde = gaussian_kde(true_samples[:, i].T, bw_method=bandwidth)
        x_vals = np.linspace(-1.90, -1.55, 100)
    elif  i ==3:
        ...
        bandwidth = 1e-6
        kde = gaussian_kde(true_samples[:, i].T, bw_method=bandwidth)
        x_vals = np.linspace(1.25, 1.55, 100)
    pdf_vals = kde(x_vals)

    # Plot the kernel density estimate
    ax.plot(x_vals, pdf_vals, color='blue', linewidth=1.4)

    ax.set_title(f'X_{i+1}', fontsize=16)
