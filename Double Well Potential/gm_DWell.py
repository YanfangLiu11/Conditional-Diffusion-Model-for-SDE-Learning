...
savedir = 'C:\\Users\\yliu1\\OneDrive - Middle Tennessee State University\\cyberduck\\sde_learning\\final_code\\DWell\\'
make_folder(savedir)

N_sample_path = 100000
x_sample,y_sample = Gendata(sde_T,sde_Nt, N_sample_path,IC_='uniform' )
...
for jj in range(ode_time_steps):
    ...
    ode_path_true = ode_path_true+(ode_path_true-ode_path_true**3)*sde_dt +sigma*np.sqrt(sde_dt)*np.random.randn(Npath,x_dim)
...
ax[1].set_yticks(np.linspace(0.46,0.54,5))
# Increase font size of tick labels
...
# x_pred_new= torch.clone((true_init*torch.ones(Npath,x_dim)).to(device))
# prediction = ( FN( (torch.hstack((x_pred_new,torch.randn(Npath,x_dim).to(device,dtype= torch.float32)))-xTrain_mean)/xTrain_std  ) * yTrain_std + yTrain_mean  +x_pred_new ).to('cpu').detach().numpy() 

# ode_path_true = true_init*np.ones((Npath,x_dim))
# print('true_path shape: ',ode_path_true.shape)
# true_samples = ode_path_true+(ode_path_true-ode_path_true**3)*sde_dt +sigma*np.sqrt(sde_dt)*np.random.randn(Npath,x_dim)
