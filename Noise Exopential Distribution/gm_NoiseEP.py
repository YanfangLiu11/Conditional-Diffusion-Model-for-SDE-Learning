...
savedir = 'C:\\Users\\yliu1\\OneDrive - Middle Tennessee State University\\cyberduck\\sde_learning\\final_code\\Noise_EP\\'
make_folder(savedir)

N_sample_path = 150000
x_sample,y_sample = Gendata(sde_T,sde_Nt, N_sample_path,IC_='uniform',mean=False)
...
for jj in range(ode_time_steps):
    ...
    ode_path_true = ode_path_true+(mu*ode_path_true)*sde_dt +sigma*np.sqrt(sde_dt)*np.random.exponential(scale=1, size=(Npath,x_dim))
...
Npath = 500000
N_x0=100
x0_grid=np.linspace(0.3,0.9,N_x0)
bx_pred = np.zeros(N_x0)
sigmax_pred = np.zeros(N_x0)
bx_true = mu*x0_grid+sigma/np.sqrt(sde_dt)
...
true_samples =  ode_path_true+(mu*ode_path_true)*sde_dt +sigma*np.sqrt(sde_dt)*np.random.exponential(scale=1, size=(Npath,x_dim))
...
ax.set_title(f'$x_0$ = {true_init:.2f}', fontsize=16)
ax.set_xlim([0.31,0.43])
ax.set_xticks(np.linspace(0.32,0.42,6))
# Increase font size of tick labels
ax.tick_params(axis='both', labelsize=14)
