...
# Function to generate paths using Euler method
def Gendata(initial,dt, N_data, N_steps,IC_):
    theta = 1.0
    sigma = 0.3
    m = 1 / np.sqrt(np.e)
    if IC_=='uniform':
        np.random.seed(2)
        xIC = np.random.uniform(0.1, 2.0, size=(N_data,1))
    elif IC_=='value':
        xIC = initial*np.ones((N_data,1))
    
    data = np.zeros((N_data, N_steps + 1))
    data[:,0]=xIC[:,0]
    for i in range(N_steps):
        data[:,i+1] = m**(dt) * data[:,i]**(1 - theta*dt) * np.random.lognormal(0, 1, size=(N_data,)) **(sigma * np.sqrt(dt))
    x_start_new = data[:,:N_steps].reshape(-1,1)
    x_end_new = data[:,1:N_steps+1].reshape(-1,1)
...
savedir = 'C:\\Users\\yliu1\\OneDrive - Middle Tennessee State University\\cyberduck\\sde_learning\\final_code\\Noise_LN\\'
make_folder(savedir)

N_sample_path = 200000
initial0 = 1.0  
x_sample,y_sample = Gendata(initial0, sde_dt, N_sample_path, sde_Nt,IC_='uniform')
...
for jj in range(ode_time_steps):
    ...
    eta = np.random.lognormal(0, 1, size=(Npath, x_dim)) **(sigma * np.sqrt(sde_dt))
    ode_path_true = m**(sde_dt) * ode_path_true**(1 - theta*sde_dt) * eta
    # print('size of ode_path_true is: ', ode_path_true.shape)
...
bx_true = np.log(m*x0_grid**(-theta))+sigma**2/2.0
sigmax_true = np.sqrt( np.exp(sigma**2*sde_dt) -1 ) * (m*np.exp(sigma**2/2.0))**(sde_dt) *x0_grid**(1-theta*sde_dt)
...
for jj in range(N_x0):
    ...
    bx_pred[jj] = np.log( (np.mean( prediction/true_init))**(1/sde_dt) )
    sigmax_pred[jj] = np.std(prediction)
...
eta = np.random.lognormal(0, 1, size=(Npath,x_dim)) **(sigma * np.sqrt(sde_dt))
true_samples =  m**(sde_dt) * ode_path_true**(1 - theta*sde_dt) * eta
...
ax.set_xticks(np.linspace(x_min+0.01,x_max+0.01,5))
# Increase font size of tick labels
ax.tick_params(axis='both', labelsize=14)
