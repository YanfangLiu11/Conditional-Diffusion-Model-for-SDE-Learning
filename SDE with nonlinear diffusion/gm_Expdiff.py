...
def Gendata(dT,dt,sde_path, interval_start, interval_end):
    torch.manual_seed(12345678)
    
    N_steps = int(dT/dt)
    t_gen = torch.linspace(0,dT, N_steps+1).reshape(1,-1).to(device)
    
    x0 = interval_start+ (interval_end-interval_start)*torch.rand( sde_path).reshape(-1,1).to(device)
    raw_data = torch.zeros(sde_path,N_steps+1).to(device)
    raw_data[:,0] = x0[:,0]
    for j in range(N_steps):
        raw_data[:,j+1] = raw_data[:,j]*(1-mu*dt) +sigma* torch.exp(-raw_data[:,j]**2)*np.sqrt(dt)*torch.randn(sde_path,x_dim).squeeze().to(device)
    x_start_new = raw_data[:,:N_steps].reshape(-1,1)
    x_end_new = raw_data[:,1:N_steps+1].reshape(-1,1)

    # plt.plot(t_gen.T.to('cpu'),raw_data.T.to('cpu'))
    # plt.savefig(os.path.join(savedir, 'raw_data.png'), dpi=300)
    # plt.show()
    
    sorted_indices = torch.argsort(x_start_new,axis=0)
...
savedir = 'C:\\Users\\yliu1\\OneDrive - Middle Tennessee State University\\cyberduck\\sde_learning\\final_code\\Exp_diff\\'
make_folder(savedir)

N_sample_path = 150000
x_sample,y_sample = Gendata(sde_T,sde_dt, N_sample_path,interval_a, interval_b)
...
for jj in range(ode_time_steps):
    ...
    ode_path_true = ode_path_true*(1-mu*sde_dt) +sigma* np.exp(-ode_path_true**2)*np.sqrt(sde_dt)*np.random.randn(Npath,x_dim)
...
bx_true = -mu*x0_grid
sigmax_true = sigma*np.exp(-x0_grid**2)
...
true_samples = ode_path_true*(1-mu*sde_dt) +sigma* np.exp(-ode_path_true**2)*np.sqrt(sde_dt)*np.random.randn(Npath,x_dim)
