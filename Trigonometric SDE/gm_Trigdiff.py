...
savedir = 'C:\\Users\\yliu1\\OneDrive - Middle Tennessee State University\\cyberduck\\sde_learning\\final_code\\Trig_diff\\'
make_folder(savedir)
...
for jj in range(ode_time_steps):
    ...
    ode_path_true = ode_path_true + np.sin(2 * k * np.pi*ode_path_true)*sde_dt + sigma* np.cos(2 * k * np.pi*ode_path_true)*np.sqrt(sde_dt)*np.random.randn(Npath,x_dim)
...
bx_true = np.sin(2*k*np.pi*x0_grid)
sigmax_true = np.abs(sigma*np.cos(2*k*np.pi*x0_grid))
...
true_samples = ode_path_true + np.sin(2 * k * np.pi*ode_path_true)*sde_dt + sigma* np.cos(2 * k * np.pi*ode_path_true)*np.sqrt(sde_dt)*np.random.randn(Npath,x_dim)
